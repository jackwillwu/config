...
const net_1 = __importDefault(__webpack_require__(3));
...
function createClient(config, selector, cwd, workspaceFolder, outputChannel) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (use_stdio && process.platform != "win32") {
            ...
            client = new coc_nvim_1.LanguageClient('r', 'R Language Server', { command: path, args: args, options: options }, clientOptions);
        }
        else {
            client = new coc_nvim_1.LanguageClient('r', 'R Language Server', tcpServerOptions, clientOptions);
        }
        return client;
    });
}
...
function activate(context) {
    const config = coc_nvim_1.workspace.getConfiguration('r');
    const outputChannel = coc_nvim_1.workspace.createOutputChannel('R Language Server');
    ...
    function didOpenTextDocument(document) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const folder = coc_nvim_1.workspace.getWorkspaceFolder(document.uri);
            ...
            if (uri.scheme === 'vscode-notebook-cell') {
                ...
                if (!checkClient(key)) {
                    ...
                    let client = yield createClient(config, documentSelector, path_1.default.dirname(uri.fsPath), folder, outputChannel);
                    client.start();
            ...
            if (folder) {
                ...
                const key = getKey(coc_nvim_1.Uri.parse(folder.uri));
                if (!checkClient(key)) {
                    console.log(`Start language server for ${document.uri.toString()}`);
                    const pattern = `${coc_nvim_1.Uri.parse(folder.uri).fsPath}/**/*`;
                    const documentSelector = [
                        { scheme: 'file', language: 'r', pattern: pattern },
                        { scheme: 'file', language: 'rmd', pattern: pattern },
                    ];
                    let client = yield createClient(config, documentSelector, coc_nvim_1.Uri.parse(folder.uri).fsPath, folder, outputChannel);
                    client.start();
                    clients.set(key, client);
                    initSet.delete(key);
                }
                ...
                if (uri.scheme === 'untitled') {
                    ...
                    if (!checkClient(key)) {
                        ...
                        let client = yield createClient(config, documentSelector, os_1.default.homedir(), undefined, outputChannel);
                        client.start();
                        clients.set(key, client);
                        initSet.delete(key);
                    }
                ...
                if (uri.scheme === 'file') {
                    ...
                    if (!checkClient(key)) {
                        ...
                        let client = yield createClient(config, documentSelector, path_1.default.dirname(uri.fsPath), undefined, outputChannel);
                        client.start();
                        clients.set(key, client);
                        initSet.delete(key);
                    }
    ...
    function didCloseTextDocument(document) {
        return __awaiter(this, void 0, void 0, function* () {
            let uri = coc_nvim_1.Uri.parse(document.uri);
            if (uri.scheme === 'untitled') {
                const result = coc_nvim_1.workspace.textDocuments.find((doc) => coc_nvim_1.Uri.parse(doc.uri).scheme === 'untitled');
                if (result) {
            ...
            if (uri.scheme === 'vscode-notebook-cell') {
                const result = coc_nvim_1.workspace.textDocuments.find((doc) => coc_nvim_1.Uri.parse(doc.uri).scheme === uri.scheme && coc_nvim_1.Uri.parse(doc.uri).fsPath === uri.fsPath);
                if (result) {
    ...
    coc_nvim_1.workspace.onDidOpenTextDocument(didOpenTextDocument, null, subscriptions);
    coc_nvim_1.workspace.onDidCloseTextDocument(didCloseTextDocument, null, subscriptions);
...
const winreg_1 = __importDefault(__webpack_require__(7));
...
,   spawn         = __webpack_require__(1).spawn
